...
export class FirebaseListObservable extends Observable {
    ...
    lift(operator) {
        const observable = new FirebaseListObservable();
    ...
    update(item, value) {
        return this._checkOperationCases(item, {
            stringCase: () => this._ref.child(item).update(value),
            firebaseCase: () => item.update(value),
            snapshotCase: () => item.ref().update(value),
            unwrappedSnapshotCase: () => this._ref.child(item.$key).update(value)
        });
    }
    remove(item = null) {
        if (!item) {
        ...
        return this._checkOperationCases(item, {
            stringCase: () => this._ref.child(item).remove(),
            firebaseCase: () => item.remove(),
            snapshotCase: () => item.ref().remove(),
            unwrappedSnapshotCase: () => this._ref.child(item.$key).remove()
        });
    }
